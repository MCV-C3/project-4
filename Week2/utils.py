...
def aggregate_predictions(outputs, original_batch_size, aggregation):
    """
    Aggregates patch predictions back to image-level predictions.
    """
    num_patches = outputs.size(0) // original_batch_size # Inputs.size(0) -> Same
    outputs_grouped = outputs.view(original_batch_size, num_patches, -1)
    
    if aggregation == "mean":
        return outputs_grouped.mean(dim=1)
    elif aggregation == "max":
        return outputs_grouped.max(dim=1).values
    elif aggregation == "median":
        return outputs_grouped.median(dim=1).values
    else:
