...
class BasicCNN(nn.Module):
    def __init__(self, num_classes, img_size, config):
        ...
        for i, out_channels in enumerate(config['channels']):
            self.features.add_module(f"conv{i}", nn.Conv2d(input_channels, out_channels, kernel_size=config['kernel_size'], padding=1))
            ...
            if config.get('dropout', 0.0) > 0:
                 self.features.add_module(f"dropout{i}", nn.Dropout2d(p=config['dropout']))
